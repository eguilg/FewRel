...
class GNN(fewshot_re_kit.framework.FewShotREModel):
    ...
    def forward(self, support, query, N, K, total_Q):
        ...
        '''
        support = self.sentence_encoder(support)  # B*N*K, hidden
        query = self.sentence_encoder(query)    # B*N*K, hidden
        support = support.view(-1, N, K, self.hidden_size)  # B, N, K, hidden
        query = query.view(-1, total_Q, self.hidden_size)  # B, total_Q, hidden

        B = support.size(0)
        # NQ = query.size(1)
        D = self.hidden_size

        support = support.unsqueeze(1).expand(-1, total_Q, -1, -1, -1).contiguous().view(-1, N * K, D) # (B * NQ, N * K, D)
        query = query.view(-1, 1, D)  # (B * NQ, 1, D)
        labels = Variable(torch.zeros((B * total_Q, 1 + N * K, N), dtype=torch.float)).cuda()
        for b in range(B * total_Q):
            for i in range(N):
                for k in range(K):
